  ...
  return (
    <>
      {Array.from({ length: numberOfSkeleton }).map((_, i) => (
        <div
          key={i}
          className={`${mode=="dark" ? ThemeMode?.dark?.cardColor : "bg-slate-100"}  rounded-lg min-h-12 relative overflow-hidden hover:shadow-md cursor-pointer h-fit w-full px-2 `}
        >
